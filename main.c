...
int main()
{
    ...
    do
    {
        ...
        switch(opcion)
        {
            ...
            case 2:
                printf("%10s %15s %15s %18s %18s %10s %10s %10s %10s\n\n","NOMBRE CLIENTE","ID MASCOTA","NOMBRE","TIPO","RAZA","PAIS","EDAD","PESO","SEXO");
                MostrarListadoMascotas(listadoMascotas,listadoClientes, QTY_MASCOTAS, QTY_CLIENTES);
            break;

            case 3:
                printf("\n %40s \n","CLIENTES CON SUS MASCOTAS");
                MostrarClientesConMascotas(listadoClientes, QTY_CLIENTES, listadoMascotas, QTY_MASCOTAS);
                printf("----------------------------------------------------------------------------------------\n");
            break;

            case 4:
                if(BuscarMascotaLibre(listadoMascotas, QTY_MASCOTAS) != -1)
                {
                    printf("%15s %15s %18s %18s %10s %10s %10s \n\n","ID","NOMBRE","APELLIDO","LOCALIDAD","EDAD","TELEFONO","SEXO");
                    MostrarListadoClientes(listadoClientes, QTY_CLIENTES);
                }

                if(AltaMascotas(listadoMascotas, QTY_MASCOTAS, idMascotas) != -1)
                {
                    idMascotas++;
                }
                else
            ...
            case 5:
                funcionRetorno = BajaMascota(listadoMascotas, QTY_MASCOTAS);
                if(funcionRetorno == -1)
                {
            ...
            case 6:
                funcionRetorno = ModificarMascota(listadoMascotas, QTY_MASCOTAS);
                if(funcionRetorno == -1)
                {
                    printf("ID Mascota no encontrada. \n");
                }
            ...
            case 12:
                ...
                MostrarMascotasMasTres(listadoMascotas, listadoClientes, QTY_MASCOTAS, QTY_CLIENTES);
            break;

            case 13:
                MostrarMascotasPorTipo(listadoMascotas, QTY_MASCOTAS);
            break;

            case 14:
                MostrarClientesConMascotaOrdenado(listadoClientes, listadoMascotas, QTY_CLIENTES, QTY_MASCOTAS);
            break;

            case 15:
                MostrarClientesPorCantidadMascotaNombre(listadoClientes, listadoMascotas, QTY_CLIENTES, QTY_MASCOTAS);
            break;
